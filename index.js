var main = new Vue({
  ...
  data: {
    ...
    resume: ["resume", "cv", "about", "aabhas", "kharel"],
    github: ["github", "code", "programming", "hack", "git"],
    education: [
      "school",
      "uni",
      "college",
    ...
    projects: ["project", "projects", "side"],
    skills: [],
    help: ["help", "confused", "how", "what"]
  },
  mounted() {
    this.scrollHandler();
    ...
    input: function(newInput, oldInput) {
      ...
      for (var i in this.education) {
        if (this.input.toLowerCase().indexOf(this.education[i]) > -1) {
          this.out =
            "I am a junior at The University of Texas at Arlington studying Software Engineering.";
          this.output = "";
          this.speed = 20;
          this.typeWriter();
        }
      ...
      for (var i in this.help) {
        if (this.input.toLowerCase().indexOf(this.help[i]) > -1) {
          this.out =
            "Available search terms: Resume, Education, Projects, Skills.";
          this.output = "";
          this.speed = 20;
          this.typeWriter();
        }
